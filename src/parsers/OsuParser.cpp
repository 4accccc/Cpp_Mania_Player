    ...
    while (std::getline(file, line)) {
        ...
        if (section == "TimingPoints") {
            ...
            if (parts.size() >= 2) {
                ...
                try {
                    ...
                    // parts[3] = sampleSet (0=default, 1=Normal, 2=Soft, 3=Drum)
                    if (parts.size() >= 4) {
                        int ss = std::stoi(parts[3]);
                        if (ss == 1) tp.sampleSet = SampleSet::Normal;
                        else if (ss == 2) tp.sampleSet = SampleSet::Soft;
                        else if (ss == 3) tp.sampleSet = SampleSet::Drum;
                        else tp.sampleSet = SampleSet::Normal;  // Default
                    }
                    // parts[5] = volume
                    if (parts.size() >= 6) {
        ...
        if (section == "HitObjects") {
            ...
            if (parts.size() >= 5) {
                ...
                bool hasWhistle = (hitsound & 2) != 0; // bit 1
                ...
                info.notes.back().hasWhistle = hasWhistle;
